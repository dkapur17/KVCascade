"""SnapKV: prefill-time observation-window attention scoring.

Reference: Li et al., "SnapKV: LLM Knows What You are Looking for Before Generation"
(2024). https://arxiv.org/abs/2404.14469

Mechanics:
  - At prefill end, score every token by mean attention from the LAST `window`
    queries (the "observation window"), max-pooled along time with kernel
    `pool_kernel` to smooth neighbouring tokens.
  - Keep the top `fp_capacity` heavy hitters plus the trailing `window` "local"
    recency tokens.
  - During decode, new tokens enter the local window (FIFO); the heavy-hitter
    set is FROZEN — no further eviction.
...
class SnapKVCache:
    """Prefill-time obs-window scoring + frozen heavy-hitter set + sliding local window."""
    ...
    # ------------------------------------------------------------------
    # Prefill compression: observation-window scoring -> top-K selection.
    # ------------------------------------------------------------------
    def _compress_prefill(self, layer_idx: int,
                           K_all: torch.Tensor, V_all: torch.Tensor,
                           positions: torch.Tensor,
                           attn_window: torch.Tensor) -> None:
        """attn_window: [B, H_q, window, T_pre]. Score selectable region [0, T_pre - W)
        using mean over W queries, then max-pool, then top-fp_capacity per head."""
        ...
        if T_pool <= 0:
            ...
            self.K  [layer_idx, :, :, :n_recent, :] = K_all[:, :, -n_recent:, :]
            self.V  [layer_idx, :, :, :n_recent, :] = V_all[:, :, -n_recent:, :]
            self.pos[layer_idx, :, :, :n_recent]    = positions[-n_recent:].view(1, 1, -1)
            self._window_cursor[layer_idx] = n_recent
        ...
        C_take = min(C, T_pool)
        top_idx = sig.topk(C_take, dim=-1, largest=True).indices       # [B, H_kv, C_take]
        idx_4d = top_idx.unsqueeze(-1).expand(-1, -1, -1, D)
        sel_K = K_all.gather(2, idx_4d)
        sel_V = V_all.gather(2, idx_4d)
        sel_pos = positions[top_idx]

        self.K  [layer_idx, :, :, :C_take, :] = sel_K
        self.V  [layer_idx, :, :, :C_take, :] = sel_V
        self.pos[layer_idx, :, :, :C_take]    = sel_pos

        # Trailing window slots: last `window` prefill tokens in order.
    ...
    def _decode_append(self, layer_idx: int,
        ...
        """Each new token overwrites the oldest slot in the local window (FIFO)."""
        T_new = K_new.shape[-2]
        if T_new == 0:
            return
        C = self.fp_capacity
...
def snapkv_attn_function(module, query, key, value, attention_mask=None,
                          scaling=None, dropout=0.0, **kwargs):
    cache: SnapKVCache = module._snapkv_cache
...
try:
    ...
    ALL_ATTENTION_FUNCTIONS["snapkv"] = snapkv_attn_function
except ImportError:
    ALL_ATTENTION_FUNCTIONS = None


def install_snapkv(model: nn.Module, cache: SnapKVCache) -> int:
    if ALL_ATTENTION_FUNCTIONS is None:
        raise RuntimeError("transformers is required for install_snapkv")
    if hasattr(model, "config"):
        _force_set_attn_impl(model.config, "snapkv")
    n = 0
    for module in model.modules():
        if hasattr(module, "config") and module is not model:
            _force_set_attn_impl(module.config, "snapkv")
        if hasattr(module, "layer_idx") and isinstance(module.layer_idx, int):
            module._snapkv_cache = cache
            n += 1
    return n
